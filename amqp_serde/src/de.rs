...
impl<'de, 'a> de::Deserializer<'de> for &'a mut Deserializer<'de> {
    ...
    where
        V: Visitor<'de>,
    {
        // should have a length field of u32 right before the content
        // the length is number of bytes of the sequence, not the number of elements
        self.parse_u32()?;
        let len = self.get_parsed_length()?;
    ...
    {
        // should have a length field of u32 right before the content
        // the length is number of bytes of the table, not the name-value pairs
        self.parse_u32()?;
        let len = self.get_parsed_length()?;
...
mod tests {
    ...
    fn test_missing_length() {
        #[derive(Deserialize)]
        struct Frame(String);
        // string alone can't be deserialized, only shortstr or longstr can
        let input = vec![0, 1, 2];
        let _: Frame = from_bytes(&input[..]).unwrap();
    }
    ...
    fn test_syntax_err() {
        #[derive(Deserialize)]
        struct Frame(Vec<u8>);
        // content length is 9, but only 1 byte follows
        let input = vec![0, 0, 0, 9, 0];
        let _: Frame = from_bytes(&input[..]).unwrap();
    }
    ...
    fn test_other_serde_data_models() {
        ...
        assert_eq!(b"beef".to_vec(), result.m_owned_bytes);
