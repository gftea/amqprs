use amqp_serde::{types::{ShortStr, LongStr}, to_buffer};
use bytes::BytesMut;
    ...
    pub(crate) fn get_response(&self) -> String {
        match self.mechanism {
            ...
            AuthenticationMechanism::AMQPLAIN => {
                ...
                to_buffer(&<&str as TryInto<LongStr>>::try_into(&self.username).unwrap(), &mut buf).unwrap();

                to_buffer(&<&str as TryInto<ShortStr>>::try_into("PASSWORD").unwrap(), &mut buf).unwrap();
                to_buffer(&'S', &mut buf).unwrap();
                to_buffer(&<&str as TryInto<LongStr>>::try_into(&self.password).unwrap(), &mut buf).unwrap();
